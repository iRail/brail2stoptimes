...
util.inherits(BRail2StopTimesTransform, Transform);

function BRail2StopTimesTransform (arrdep, stationid, nextStopTimeResolver) {
  Transform.call(this, {objectMode: true});
  this._arrdep = arrdep;
  this._stationid = stationid;
  this.nextStopTimeResolver = nextStopTimeResolver;
}

/**
 * Function puts all the XML straight into a stoptime object
 */
BRail2StopTimesTransform.prototype._transform = function (journey, encoding, done) {
  ...
  */
  ...
  self.nextStopTimeResolver.promiseNextStopTime(stoptime).then(function (nextStopTime) {
    ...
    console.log("No nextstoptime: ", error)
    self.push(stoptime);
    done();
  });
}
